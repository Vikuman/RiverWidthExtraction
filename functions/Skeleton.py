...
from functions.stream import Stream
...
class Skeleton:
	...
	def __init__(self,river,len_dang_arcs,fast):
		...
		list_all_skeleton_points=[]
		(self.row,self.col) = self.array_skeleton.shape
		for i in range(self.row):
			for j in range(self.col):
				if self.isSkeleton(i,j):
					_,list_next_points = self.isJunctionAndNextPoint(i,j,0)
					list_all_skeleton_points.append(list_next_points)
		with open("skeleton_with_arcs.csv", "+w") as f:
			writer = csv.writer(f)
			writer.writerows(list_all_skeleton_points)
		f.close()	
		...
		# plt.savefig('temp/10_Skeleton.png', format = 'png', dpi=1200)
		# ---------------------------------------------------------------------
			...
			for i in range(self.row):
				for j in range(self.col):
					if self.isSkeleton(i,j):
						_,list_next_points = self.isJunctionAndNextPoint(i,j,0)
						list_all_skeleton_points.append(list_next_points)
			with open("skeleton_without_arcs.csv", "+w") as f:
				writer = csv.writer(f)
				writer.writerows(list_all_skeleton_points)
			f.close()	
			plt.imshow(self.array_skeleton)
	...
	def isJunctionAndNextPoint(self,x,y,to_return):
		...
		list_temporary =[]#made to swap delete it
