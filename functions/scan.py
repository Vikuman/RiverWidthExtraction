...
from functions.reach import Reach
from functions.Skeleton import Skeleton
from functions.junction import Junction
...
class Scan:
	...
	def getAllInfo(self,stream):
		...
		# total_no_of_pixels,no_of_pixel_on_right,no_of_pixel_on_left,no_of_pixels_on_right,no_of_pixels_on_left = 0,0,0,0,0
	...
	def getPerpPoint(self,x,y,l):
		...
		for i in list(reversed(range(0,3))):
			for j in list(reversed(range(-2,3))):
				...
				if (self.skeleton.isSkeleton(x-l*i,y-l*j)>0): 
					perpt_x = x-l*i
					perpt_y = y-l*j
					return x-l*i,y-l*j
	...
	def getPerpendicularLine(self,x1,y1,x2,y2,x0,y0):
		...
		return m1,c1
	# -------------------------------------------------------------------------
	# find the nextpoints of a given river-boundary point (used in to create csv file for list of boundary points)

	def GetNextPoint_riverBoundary(self,x,y):
		cur=(x,y)
		list_nextPoints=[] # to store the points next to the current river boundary point
		nextP=[] # to store the 4 points surrounding the given  point
		...
		for j in range(len(nextP)):
			# find all the neighbouring points that belong to the river boundary
			# append them to the list - list_nextPoints
			pos = nextP[j]
			if pos[0]<0 or pos[0]>=self.skeleton.row or pos[1]<0 or pos[1]>=self.skeleton.col: continue
			if not self.river.isBoundary(pos[0],pos[1]): continue
			list_nextPoints.append(pos)
			flag[j]=True
				
		# now find all the neighbouring diagonal points that belong to the skeleton
		# append them to the list - list_nextPoints
		if flag[0]==False and flag[2]==False and self.river.isBoundary(cur[0]-1,cur[1]-1):
			list_nextPoints.append([cur[0]-1,cur[1]-1])
			
		if flag[0]==False and flag[3]==False and self.river.isBoundary(cur[0]+1,cur[1]-1):
			list_nextPoints.append([cur[0]+1,cur[1]-1])
		
		if flag[1]==False and flag[2]==False and self.river.isBoundary(cur[0]-1,cur[1]+1):
			list_nextPoints.append([cur[0]-1,cur[1]+1])
			
		if flag[1]==False and flag[3]==False and self.river.isBoundary(cur[0]+1,cur[1]+1):
			list_nextPoints.append([cur[0]+1,cur[1]+1])
	...
	def averageCalculation(self, interval_distance, lol_of_stream_points):
		list_next_points = []		#stores the points next to each other in the river boundary
		list_all_river_boundary_points = []		#stores the list of 'list_next_points' i.e the entire river boundary in form of list of list
		...
		for i in range(self.skeleton.row):
			...
			for j in range(self.skeleton.col): # for each pixel (i,j)
				...
				if self.river.isBoundary(i,j):
					flag=True
					#find the list of next river boundary points to the current river-boundary point
					# list_next_points = self.GetNextPoint_riverBoundary(i,j)
					# list_all_river_boundary_points.append(list_next_points)
		...
		for i in range(len(lol_of_stream_points)):
			for j in range(len(lol_of_stream_points[i])):
				...
				counter+=1 # increment the counter
		# ---------------------------------------------------------------------
		# create a csv file for the river boundary points
		# with open("river_boundary_point.csv", "wb") as f:
		# 	writer = csv.writer(f)
		# 	writer.writerows(list_all_river_boundary_points)
		# f.close()
		#--------------------------------------------------------------	
	...
	def drawStream(self,a_Image,l_Stream,color,list_all_green_lines):
		for list_temp in l_Stream:
			for stream in list_temp:
				stream.Draw(a_Image,color,list_all_green_lines)
		return
	# -------------------------------------------------------------------------
	# -----------------------------------------------------------------------------------
