...
class River:
	...
	def __init__(self,img,threshVal,interval,fast):
		...
		if not fast:
			...
			self.FindIsolatedLandArea(interval)
		return
	# -----------------------------------------------------------------------------------
	...
	def RotateAndCrop(self,img,threshVal):
		...
		self.array_Boundary = copy.copy(self.array_Image) # create and store its shallow copy in array_Boundary
	...
	def FindIsolatedLandArea(self,interval):
		...
		for i in range(0,self.row,interval): # mark the sections using blue lines with the interval equal to - interval
		...
		for i in range(self.row):
			...
			if i>temp_interval:
				...
				temp_interval+=interval # calculate the next interval line

			# if we are on the interval line, skip the ith iteration
			elif i==temp_interval:
				break
	...
	def getContour(self):
		self.array_Boundary = copy.copy(self.array_Image) # create a shallow copy of the numpy array - array_Image, and store it in array_Boundary
